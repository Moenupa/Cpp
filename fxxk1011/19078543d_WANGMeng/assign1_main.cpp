...
class NewOrder {
    ...
    public:
        ...
        int cal_total()                                 // calculate total order price
        {
...
int main () {
    ...
    ORDIN:                                              // order: ask for item code
        ...
        if (orders[cur_cus].order.find(tempOrd) != orders[cur_cus].order.end()) {
            orders[cur_cus].order[tempOrd] += 1;        // if valid add the food by 1
    ...
    ORDLOOP:                                            // orderloop: continue or stop ordering
        ...
        if (cmd.compare("0") == 0)                      // stop: save the order, then return to sysinput
        {
            ...
            printCmd("-R");
            for (auto iter = orders[cur_cus].order.begin(); iter != orders[cur_cus].order.end(); iter++) {                                  // go through all ITEM
...
void printCmd(string token)                             // print Command details
{
    ...
    {
        buf = INFO[i];
        if (buf.compare(0,2,token)==0)                  // if token detected, start or end printing
        {flag = !flag;}
